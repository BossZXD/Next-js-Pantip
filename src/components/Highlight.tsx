...
const highlights = jsonData.map((item, index) => ({
  id: index + 1,
  title: item.name,
...
const Highlight: React.FC = () => {
  ...
  const highlightState = useSelector((state: RootState) => state.highlight.highlights);
  ...
  return (
    <div className="flex items-center justify-center bg-[#53507c] text-white p-8 md:p-12 lg:p-16">
      <div className="relative w-full max-w-5xl">
        <h2 className="text-lg md:text-xl font-bold mb-4 text-center py-2">Highlight</h2>
        <div className="relative flex justify-center px-2">
          ...
          <div ref={scrollRef} className="flex overflow-hidden space-x-4 md:space-x-8 lg:space-x-4 justify-start">
            {highlightState.map((highlight) => (
              <a key={highlight.id} href={highlight.link} target="_blank" rel="noopener noreferrer" className="flex flex-col items-start space-y-2 flex-shrink-0 w-64 sm:w-72 md:w-80 bg-gray-800 rounded-lg p-4 hover:bg-gray-700 transition duration-300">
                <img src={highlight.image} alt={highlight.title} className="w-full h-40 sm:h-48 md:h-56 object-cover rounded-lg" />
                <h3 className="text-sm md:text-md font-bold text-center overflow-hidden text-ellipsis w-full">{highlight.title}</h3>
                <p className="text-xs md:text-sm text-center overflow-hidden text-ellipsis w-full">{highlight.description}</p>
              </a>
            ))}
          </div>
        </div>

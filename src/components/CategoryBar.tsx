...
// Import icons
import { 
  Utensils, Map, MessageCircle, Camera, Film, ShoppingBag, Heart, Home, Car, Activity, 
...
type Category = {
  ...
  pinned_time: any;
  order: any;
  icon: LucideIcon | null;  // Adjust to include icon
};
...
const iconMap: Record<string, LucideIcon> = {
  ...
  tvshow: Film,
  bangrak: Heart,
  horoscope: MessageCircle,
  writer: MessageCircle,
  chalermkrung: Film,
  chalermthai: Film,
  greenzone: Activity,
  region: Map,
  rajdumnern: Briefcase,
  religious: MessageCircle,
  supachalasai: Activity,
  siam: Home,
  isolate: MessageCircle,
  theoldsiam: Heart,
  all: Map,
};

const categories: Category[] = jsonData.map(item => ({
  ...item,
  icon: iconMap[item.slug] || null,
}));
...
const CategoryBar: React.FC = () => {
  ...
  return (
    <div className="relative bg-[#53507c] shadow-md p-4 flex justify-center">
      {showLeftArrow && (
        <button onClick={() => scroll('left')} className="absolute left-2 top-1/3 transform -translate-y-1/2 bg-white rounded-full p-1 shadow-md z-10">
          <ChevronLeft size={24} className="text-gray-600" />
        </button>
      )}
      {showRightArrow && (
        <button onClick={() => scroll('right')} className="absolute right-2  top-1/3  transform -translate-y-1/2 bg-white rounded-full p-1 shadow-md z-10">
      ...
      >
        {categories.map((category) => (
          <a key={category.id} href={category.link_url} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center space-y-2 flex-shrink-0">
